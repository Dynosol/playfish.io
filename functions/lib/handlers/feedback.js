...
exports.submitFeedback = (0, https_1.onCall)({ cors: corsOrigins, invoker: 'public' }, async (request) => {
    if (!request.auth) {
    ...
    if (trimmedMessage.length > 2000) {
        throw new https_1.HttpsError('invalid-argument', 'message is too long (max 2000 characters)');
    }
    // Check rate limit (1 per minute)
    await (0, rateLimiter_1.checkRateLimit)(userId, 'feedback:submitFeedback');
